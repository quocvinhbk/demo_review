...
class ReviewScraper
  attr_reader :latest_retrieval_from_date, :url, :output_dir, :category_names, :driver, :headless_mode, :read_timeout, :waiting_timeout,
  ...
  def initialize(latest_retrieval_from_date, output_dir, url = '')
    raise ArgumentError, 'Invalid URL' if url.empty?
  ...
  def call
    ...
    Selenium::WebDriver::Wait.new(timeout: waiting_timeout).until do
      driver.execute_script('return document.readyState') == 'complete' &&
        driver.find_elements(:xpath, overview_tab_xpath).any?
    end
    unless driver.find_elements(:xpath, reviews_tab_xpath).any?
      AppLogger.call('@@@@@ Not found Reviews tab. Exiting ReviewScraper#call.', { empty_line: true })
      return
    end
    ...
    file_name = File.join(Dir.pwd, output_dir, "#{category_names.first}.json")
    ...
    loop do
      ...
      reviews.each do |review|
        break if review_before_latest_retrieval_from_date?(review)
    ...
    category_names[1..]&.each do |category_name|
      sub_data = []
      # sub_data = data.select { |review| review[:review_description]&.include?(category_name) }
      file_name = File.join(Dir.pwd, output_dir, "#{category_name.gsub(' ', '_')}.json")
      save_to_json(file_name, sub_data)
    end
  ...
  def review_before_latest_retrieval_from_date?(review)
    ...
    Date.parse(review_date) < Date.parse(latest_retrieval_from_date)
  end
